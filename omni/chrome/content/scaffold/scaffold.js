...
var Scaffold = new function () {
  ...
  this.load = async function (translatorID) {
    ...
    if (translator.translatorType & Zotero.Translator.TRANSLATOR_TYPES.import) {
      ...
      if (translator.target.includes('xml')) {
        language = 'xml';
      }
    } else
    if (translator.translatorType & Zotero.Translator.TRANSLATOR_TYPES.search) {
      language = 'json';
    }
  ...
  async function _run(functionToRun, input, selectItems, itemDone, detectHandler, done) {
    ...
    if (functionToRun == "detectImport" || functionToRun == "doImport") {
      translate = new Zotero.Translate.Import();
      translate.setString(input);
    } else
    if (functionToRun == "doExport") {
      translate = new Zotero.Translate.Export();
      translate.setItems(input);
    } else
    if (functionToRun == "detectSearch" || functionToRun == "doSearch") {
  ...
  async function _getInput(typeOrMethod) {
    ...
    if (typeOrMethod.endsWith('import')) {
      return _getImport();
    } else
    if (typeOrMethod.endsWith('export')) {
      return _getExport();
    } else
    if (typeOrMethod.endsWith('search')) {
      return _getSearch();
    }
    return null;
  }
