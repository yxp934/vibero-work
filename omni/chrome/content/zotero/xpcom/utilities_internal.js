  ...
  renderItemTitle(title, targetNode = null) {
    ...
    for (let token of title.split(/(<[^>]+>)/)) {
      if (this._titleMarkup.hasOwnProperty(token)) {
        ...
        if (markup.endsTag && markupStack.some((otherMarkup) => otherMarkup.beginsTag === markup.endsTag)) {
...
Zotero.Utilities.Internal.activate = new function () {
  ...
  return function (win) {
    ...
    if (Zotero.isLinux && !isWayland && win) {
      const { ctypes } = ChromeUtils.importESModule("resource://gre/modules/ctypes.sys.mjs");
    ...
    if ((Zotero.isWin || Zotero.isLinux) && win) {
