...
class EditorInstance {
  ...
  async _openCitationDialog(nodeID, citationData, filterLibraryIDs, openedEmpty) {
    ...
    CI.prototype = {
       ...
       */accept: async function (progressCallback) {// Zotero.debug('CI: accept');
...
class EditorInstanceUtilities {
  ...
  serializeAnnotations(annotations, skipEmbeddingItemData) {
    ...
    for (let annotation of annotations) {
      ...
      let template;
      if (['highlight', 'underline', 'card'].includes(annotation.type)) {
      ...
      // 确保template不为undefined
      if (!template) {
        template = '<p>{{highlight}} {{citation}} {{comment}}</p>';
      }
