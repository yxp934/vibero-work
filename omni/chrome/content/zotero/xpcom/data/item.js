    ...
    set: function (value) {
      ...
      switch (name) {
        case 'type':{
            ...
            if (!['highlight', 'underline', 'note', 'text', 'image', 'ink', 'card'].includes(value)) {
...
Zotero.Item.prototype._eraseData = async function (env) {
  ...
  if (this.isAttachment()) {
    ...
    if (this.isFileAttachment()) {
      ...
      // Delete VibeDB data for this attachment (PDF解析数据、VibeCards等)
      try {
        console.log("[item.js] 等待进入删除vibedb数据");
        // 等待 VibeDB 初始化完成
        if (Zotero.VibeDB) {
          await Zotero.VibeDB.schemaUpdatePromise;
          console.log("[item.js] 正在进入删除vibedb数据");
          // 删除该论文在 VibeDB 中的所有数据（级联删除）
          const deleted = await Zotero.VibeDB.Papers.deleteAll(this.id);
          if (deleted) {
            console.log("[item.js] 已删除vibedb数据");
            Zotero.debug(`[Item] Deleted VibeDB data for item ${this.id}`);
          }
        }
      }
      catch (e) {
        // VibeDB 删除失败不应阻止 item 删除流程
        console.error(`[item.js] Failed to delete VibeDB data for item ${this.id}: ${e}`, 2);
        Zotero.logError(e);
      }
