...
var ZoteroPane = new function () {
  ...
  function setUpKeyboardNavigation() {
    ...
    let moveFocus = function (actionsMap, event, verticalArrowIsTab = false) {
      ...
      if (verticalArrowIsTab && key == 'ArrowDown' && !onInput) {
        key = 'Tab';
      }
      if (key == Zotero.arrowPreviousKey) {
        key = 'ArrowPrevious';
      } else
      if (key == Zotero.arrowNextKey) {
        key = 'ArrowNext';
      }
  ...
  async function _loadPane() {
    ...
    // ===== VibeZotero: 启动时确保有 pre-vibe 备份 =====
    // 每次启动检查：如果数据目录没有 pre-vibe 备份，则创建一个
    // 这确保用户始终有一个"干净的"可恢复点
    var preVibeBackupExists = false;
    try {
      let dbPath = PathUtils.join(Zotero.DataDirectory.dir, 'zotero.sqlite');
      let backupPath = dbPath + '.pre-vibe.bak';

      preVibeBackupExists = await IOUtils.exists(backupPath);

      if (!preVibeBackupExists) {
        console.log('[Vibero] 未检测到 pre-vibe 备份，正在创建初始备份...');
        await Zotero.DB.backUpDatabase({
          suffix: 'pre-vibe',
          force: true
        });
        console.log('[Vibero] ✓ 初始备份已创建: zotero.sqlite.pre-vibe.bak');
        preVibeBackupExists = true;
      } else {
        console.log('[Vibero] pre-vibe 备份已存在，跳过备份');
      }
    } catch (backupErr) {
      console.warn('[Vibero] ⚠ 启动备份检查失败:', backupErr);
    }

    // ✅ VibeZotero: 在 UI 初始化之前插入 Demo
    // 这样可以避免竞态条件，用户看到的就是完整的 Demo 数据
    var shouldSelectDemo = false;
    var demoCollectionID = null;
    try {
      var demoInserted = Zotero.Prefs.get('vibeZotero.demoInserted');
      if (!demoInserted) {
        // 同步插入 Demo（不选择集合）
        // 注意：如果上面已经创建了备份，就跳过 Demo 插入时的备份
        var result = await ZoteroPane.insertVibeZoteroDemo(true, { skipBackup: preVibeBackupExists });
        if (result && result.collectionID) {
          shouldSelectDemo = true;
          demoCollectionID = result.collectionID;
          Zotero.Prefs.set('vibeZotero.demoInserted', true);
        }
      }
    } catch (e) {
      // Demo 插入失败不影响主应用启动
      console.error('[Vibero] 插入 Demo 失败，继续正常启动:', e);
    }
    ...
    // switching data directory locations)
      ...
      if (Zotero.Schema.dbInitialized || !Zotero.Sync.Server.enabled) {
        setTimeout(function () {
          ...
          // ZoteroPane_Local.loadURI(ZOTERO_CONFIG.START_URL); // Commented out to prevent opening zotero.org on startup
        }, 400);}
    ...
    // ✅ 如果首次运行已插入 Demo，在 UI 就绪后选择 Demo 集合
    if (shouldSelectDemo && demoCollectionID) {
      setTimeout(async function () {
        try {
          console.log('[Vibero] 自动选择 Demo 集合:', demoCollectionID);
          await ZoteroPane.collectionsView.selectCollection(demoCollectionID);
          console.log('[Vibero] ✓ Demo 集合已选中');
        } catch (e) {
          console.error('[Vibero] 选择 Demo 集合失败:', e);
        }
      }, 100);
    }

    // console.log('[Vibero] ########## zoteroPane.js onLoad 执行中 ##########');

    setTimeout(async function () {
      // console.log('[Vibero] ########## setTimeout 回调开始执行 ##########');
  ...
  function captureKeyDown(event) {
    ...
    // Toggle context pane: Cmd/Ctrl+L
    if (cmdOrCtrlOnly && event.key.toLowerCase() === 'l') {
      ZoteroPane.toggleContextPane();
      event.preventDefault();
      event.stopPropagation();
      return;
    }

    // Toggle collections pane (sidebar): Cmd/Ctrl+B
    if (cmdOrCtrlOnly && event.key.toLowerCase() === 'b') {
      ZoteroPane.toggleCollectionsPane();
      event.preventDefault();
      event.stopPropagation();
      return;
    }
  ...
  this.deleteSelectedItems = function (force, fromMenu) {
    ...
    if (collectionTreeRow.isTrash() || collectionTreeRow.isBucket()) {
      prompt = toDelete;
    }
  ...
  this.toggleCloudSyncPanel = async function (button) {
    ...
    // 关键点：弹出后给 iframe 强制焦点，否则无法响应键盘（Zotero XUL 常见的坑）
    setTimeout(() => {
  ...
  this._initCloudSyncPanel = async function (panel) {
    ...
    iframe.setAttribute('id', 'account-status-iframe'); // 设置 ID 以便其他模块查找
   ...
   * @param {Element} panel - 面板元素
   */
  this._updateCloudSyncPanelStatus = async function (panel) {
    if (!panel._iframe || !panel._iframe.contentWindow) {
      return;
    }
    ...
    try {
      // 通知 iframe 检查登录状态
      if (panel._iframe.contentWindow.syncPaneAPI) {
        await panel._iframe.contentWindow.syncPaneAPI.checkAuthStatus();
      }
    } catch (e) {
      Zotero.logError(e);
    }
  ...
  this.buildCollectionContextMenu = async function () {
    ...
    if (collectionTreeRow.isDuplicates() || collectionTreeRow.isUnfiled() || collectionTreeRow.isRetracted()) {
  ...
  this.newNote = async function (popup, parentKey, text, citeURI) {
    ...
    if (this.getCollectionTreeRow().isCollection()) {
  ...
  this.viewAttachment = Zotero.serial(async function (itemIDs, event, noLocateOnMissing, extraData) {
    ...
    var launchFile = async (path, item) => {
      ...
      if (contentType === 'application/epub' && (await sniffType()) === 'application/epub+zip') {
  ...
  */
  ...
  this.addNoteFromAnnotationsFromSelected = async function () {
    ...
    for (let item of items) {
      ...
      if (isAttachmentWithExtractableAnnotations(item)) {
        attachments.push(item);
      } else
      if (item.isAnnotation() && item.annotationType != 'ink') {
  ...
  this.addNotesFromAnnotationsFromSelected = async function () {
    ...
    for (let item of items) {
      ...
      if (item.isFileAttachment() && !item.isTopLevelItem()) {
        attachments.push(item);
      } else
      if (items.length == 1) {
  ...
  /**
   * VibeZotero: 检查是否是首次运行，如果是则插入demo论文
   */
  this.checkVibeZoteroFirstRun = async function () {
    console.log('[Vibero] ========== 函数 checkVibeZoteroFirstRun 被调用 ==========');
    console.log('[Vibero] ========== 开始首次运行检测 ==========');

    // 检查是否已经插入过demo
    var demoInserted = Zotero.Prefs.get('vibeZotero.demoInserted');
    console.log('[Vibero] 检查pref vibeZotero.demoInserted: ' + demoInserted);

    if (demoInserted) {
      console.log('[Vibero] Demo已经插入过，跳过');
      return;
    }

    try {
      console.log('[Vibero] 首次运行，准备插入demo论文');
      console.log('[Vibero] 当前 itemsView 状态:', this.itemsView ? '存在' : '不存在');
      if (this.itemsView) {
        console.log('[Vibero] itemsView._initialized:', this.itemsView._initialized);
      }
      var success = await this.insertVibeZoteroDemo();

      // 只有成功插入才标记已完成
      if (success) {
        Zotero.Prefs.set('vibeZotero.demoInserted', true);
        console.log('[Vibero] ✓ 设置pref vibeZotero.demoInserted = true');
        console.log('[Vibero] ========== Demo论文插入流程完成 ==========');
      } else
      {
        console.warn('[Vibero] ✗ Demo论文插入失败，不设置pref');
      }
    }
    catch (e) {
      console.error('[Vibero] ✗ 插入demo论文失败:', e);
      console.error('[Vibero] 错误堆栈:', e.stack);
    }
  };


  /**
   * VibeZotero: 安全插入 Demo 论文
   * 
   * 安全策略：
   * 1. 执行前强制备份数据库（确保可恢复）
   * 2. 添加超时保护（防止卡死）
   * 3. 失败不影响主应用
   * 
   * @param {boolean} skipAutoSelect - 是否跳过自动选择集合（默认false）
   * @param {Object} options - 可选配置
   * @param {number} options.timeout - 超时时间（毫秒），默认 60000
   * @param {boolean} options.skipBackup - 是否跳过备份（默认 false）
   * @returns {Object|boolean} 成功时返回 {collectionID, itemID}，失败返回 false
   */
  this.insertVibeZoteroDemo = async function (skipAutoSelect = false, options = {}) {
    const TIMEOUT = options.timeout || 60000; // 默认 60 秒超时
    const SKIP_BACKUP = options.skipBackup || false;

    try {
      console.log('[Vibero] ========== insertVibeZoteroDemo 开始 ==========');

      // ===== 安全措施1: 强制备份数据库 =====
      if (!SKIP_BACKUP) {
        console.log('[Vibero] --- 安全措施: 备份数据库 ---');
        try {
          // 1. 检查是否存在旧的 pre-vibe 备份，如果存在则重命名为 .1.bak, .2.bak
          let dbPath = Zotero.DB.dbPath; // 修正：直接访问 DB.dbPath 可能不可靠，改用 safe getter
          if (!dbPath) {
            dbPath = PathUtils.join(Zotero.DataDirectory.dir, 'zotero.sqlite');
          }
          ...
          if (await IOUtils.exists(backupPath)) {
            console.log('[Vibero] 检测到已存在 pre-vibe 备份，正在执行增量重命名...');

            let i = 1;
            let newPath;
            // 寻找下一个可用的编号
            do {
              newPath = dbPath + '.pre-vibe.' + i + '.bak';
              i++;
            } while (await IOUtils.exists(newPath));

            // 重命名旧备份
            await IOUtils.move(backupPath, newPath);
            console.log('[Vibero] 旧备份已重命名为: ' + PathUtils.filename(newPath));
          }

          // 2. 执行新的备份
          // 使用 Zotero 内置的数据库备份机制
          await Zotero.DB.backUpDatabase({
            suffix: 'pre-vibe',
            force: true
          });
          console.log('[Vibero] ✓ 数据库已备份为 zotero.sqlite.pre-vibe.bak');
        } catch (backupErr) {
          // 备份失败不阻止继续，但记录警告
          console.warn('[Vibero] ⚠ 数据库备份失败，继续执行:', backupErr);
        }
      }

      console.log('[Vibero] --- 步骤1: 构建demo PDF路径 ---');

      // 使用 resource:// 协议访问 resource 目录下的 demo PDF
      var demoPdfURI = 'resource://zotero/demo/demo-paper.pdf';
      console.log('[Vibero] Demo PDF URI: ' + demoPdfURI);

      // 将 resource:// URI 解析为实际的 URI（可能是 jar: 或 file:）
      var ioService = Services.io;
      var resourceURI = ioService.newURI(demoPdfURI);

      // 使用 resolveURI 将 resource:// 解析为实际 URI
      var resolvedURI = ioService.newURI(resourceURI.spec);
      if (resourceURI.scheme === 'resource') {
        var resProt = ioService.getProtocolHandler('resource').
        QueryInterface(Components.interfaces.nsIResProtocolHandler);
        resolvedURI = resProt.resolveURI(resourceURI);
      }
      console.log('[Vibero] 解析后的 URI: ' + resolvedURI);

      // 如果是 jar: URI，需要先提取到临时目录
      var demoPdfPath;
      if (resolvedURI.startsWith('jar:')) {
        console.log('[Vibero] 检测到 JAR 文件，提取到临时目录');
        var tempDir = Zotero.getTempDirectory().path;
        demoPdfPath = PathUtils.join(tempDir, 'vibeZotero-demo-paper.pdf');
        console.log('[Vibero] 临时文件路径: ' + demoPdfPath);

        // 使用 Zotero.File.download 从 JAR 中提取文件
        await Zotero.File.download(resolvedURI, demoPdfPath);
        console.log('[Vibero] ✓ 文件提取成功');
      } else
      {
        // 如果是 file: URI，直接转换为文件路径
        var fileHandler = ioService.getProtocolHandler('file').
        QueryInterface(Components.interfaces.nsIFileProtocolHandler);
        var demoPdfFile = fileHandler.getFileFromURLSpec(resolvedURI);
        demoPdfPath = demoPdfFile.path;
      }
      console.log('[Vibero] Demo PDF文件路径: ' + demoPdfPath);

      // 检查demo文件是否存在 (使用IOUtils而不是OS.File)
      console.log('[Vibero] --- 步骤2: 检查文件是否存在 ---');
      var fileExists = await IOUtils.exists(demoPdfPath);
      console.log('[Vibero] 文件存在: ' + fileExists);

      if (!fileExists) {
        console.log('[Vibero] ✗ Demo PDF文件不存在，跳过插入');
        console.log('[Vibero] 提示: 请将demo PDF文件放置到: ' + demoPdfPath);
        return false;
      }

      // 创建"VIBERO DEMO"集合
      console.log('[Vibero] --- 步骤3: 创建集合 ---');
      var collection;
      try {
        collection = new Zotero.Collection();
        collection.name = '🎯 VIBERO DEMO';
        collection.parentID = null;
        console.log('[Vibero] 准备保存集合: ' + collection.name);

        await collection.saveTx();
        console.log('[Vibero] ✓ 集合创建成功, ID: ' + collection.id);

        // 验证集合是否真的保存了
        var savedCollection = Zotero.Collections.get(collection.id);
        if (!savedCollection) {
          throw new Error('集合保存后无法获取，可能保存失败');
        }
        console.log('[Vibero] ✓ 集合验证成功: ' + savedCollection.name);
      }
      catch (e) {
        console.error('[Vibero] ✗ 创建集合失败:', e);
        console.error('[Vibero] 错误堆栈:', e.stack);
        return false;
      }

      // 导入demo PDF到该集合
      console.log('[Vibero] --- 步骤4: 导入PDF ---');
      console.log('[Vibero] 导入参数:');
      console.log('[Vibero]   - file: ' + demoPdfPath);
      console.log('[Vibero]   - libraryID: ' + Zotero.Libraries.userLibraryID);
      console.log('[Vibero]   - collections: [' + collection.id + ']');
      ...
      try {
        attachment = await Zotero.Attachments.importFromFile({
          ...
          title: 'VIBERO DEMO 论文'
        });

        console.log('[Vibero] ✓ PDF导入成功, attachment ID: ' + attachment.id);

        // 验证附件是否真的保存了
        var savedAttachment = Zotero.Items.get(attachment.id);
        if (!savedAttachment) {
          throw new Error('附件保存后无法获取，可能保存失败');
        }
        console.log('[Vibero] ✓ 附件验证成功: ' + savedAttachment.getField('title'));

        // 等待一小段时间，确保数据库操作完全完成
        await Zotero.Promise.delay(100);
      }
      catch (e) {
        console.error('[Vibero] ✗ 导入PDF失败:', e);
        console.error('[Vibero] 错误堆栈:', e.stack);

        // ===== 补偿事务：回滚已创建的 Collection =====
        // 由于 Zotero 的事务边界限制，Collection 和 Attachment 无法放入同一事务
        // 但我们可以在失败时手动删除 Collection，避免留下"孤立"的空集合
        console.log('[Vibero] 执行补偿回滚：删除已创建的集合...');
        try {
          await collection.eraseTx();
          console.log('[Vibero] ✓ 集合已回滚删除');
        } catch (rollbackErr) {
          console.error('[Vibero] ⚠ 集合回滚失败:', rollbackErr);
        }

        return false;
      }

      // 步骤5: 插入预解析的 vibeDB 数据（在选中集合之前完成，避免竞态条件）
      console.log('[Vibero] --- 步骤5: 插入预解析数据到 vibeDB ---');
      try {
        await this.insertDemoVibeDBData(attachment.id);
        console.log('[Vibero] ✓ vibeDB 数据插入成功');
      }
      catch (e) {
        console.error('[Vibero] ✗ vibeDB 数据插入失败:', e);
        // vibeDB 数据插入失败不影响整体成功
      }

      // 步骤6: 自动选中该集合（可选）
      console.log('[Vibero] --- 步骤6: 选中集合 ---');
      if (!skipAutoSelect) {
        try {
          if (this.collectionsView) {
            await this.collectionsView.selectCollection(collection.id);
            console.log('[Vibero] ✓ 已自动选中集合');

            // 等待条目视图刷新完成（而不仅仅是等待初始加载）
            // selectCollection 会触发 onCollectionSelected -> itemsView.changeCollectionTreeRow -> refresh
            // 需要等待 _refreshPromise 而不是 waitForLoad，因为 waitForLoad 只等待组件初始挂载
            if (this.itemsView) {
              console.log('[Vibero] 等待条目视图刷新完成...');
              // 先等待初始加载
              await this.itemsView.waitForLoad();
              // 再等待刷新操作完成
              if (this.itemsView._refreshPromise) {
                await this.itemsView._refreshPromise;
              }
              console.log('[Vibero] ✓ 条目视图刷新完成');
            }
          } else
          {
            console.log('[Vibero] collectionsView不可用，跳过自动选中');
          }
        }
        catch (e) {
          console.error('[Vibero] ✗ 选中集合失败:', e);
          // 选中失败不影响整体成功
        }
      } else {
        console.log('[Vibero] ⊗ 跳过自动选择集合（将在UI初始化后手动选择）');
      }

      console.log('[Vibero] ========== insertVibeZoteroDemo 完成 ==========');
      return {
      ...
      console.error('[Vibero] ✗ insertVibeZoteroDemo 整体失败:', e);
      console.error('[Vibero] 错误堆栈:', e.stack);
      return false;
    }
  };
  ...
  /**
   * VibeZotero: 插入demo论文的预解析vibeDB数据
   * @param {number} itemID - PDF附件的itemID
   */
  this.insertDemoVibeDBData = async function (itemID) {
    console.log('[Vibero] 开始插入 vibeDB 数据, itemID:', itemID);

    try {
      // 1. 读取预解析数据文件
      var demoDataURI = 'resource://zotero/demo/demo-paper-vibedb.json';
      console.log('[Vibero] 数据文件 URI:', demoDataURI);

      // 解析 URI
      var ioService = Services.io;
      var resourceURI = ioService.newURI(demoDataURI);
      var resProt = ioService.getProtocolHandler('resource').
      QueryInterface(Components.interfaces.nsIResProtocolHandler);
      var resolvedURI = resProt.resolveURI(resourceURI);
      console.log('[Vibero] 解析后的 URI:', resolvedURI);
      ...
      if (resolvedURI.startsWith('jar:')) {
        // 从 JAR 中读取
        console.log('[Vibero] 从 JAR 读取数据...');
        var tempPath = PathUtils.join(Zotero.getTempDirectory().path, 'vibeZotero-demo-data.json');
        await Zotero.File.download(resolvedURI, tempPath);
        demoDataJSON = await Zotero.File.getContentsAsync(tempPath);
      } else
      {
        // 从文件系统读取
        var fileHandler = ioService.getProtocolHandler('file').
        QueryInterface(Components.interfaces.nsIFileProtocolHandler);
        var dataFile = fileHandler.getFileFromURLSpec(resolvedURI);
        demoDataJSON = await Zotero.File.getContentsAsync(dataFile.path);
      }

      // 3. 解析 JSON
      var demoData = JSON.parse(demoDataJSON);

      // 4. 等待 VibeDB 初始化完成
      await Zotero.VibeDB.schemaUpdatePromise;
      console.log('[Vibero] ✓ VibeDB 已就绪');

      // 5. 保存 paper 数据（包含 blockMapping）
      console.log('[Vibero] 保存 paper 数据...');
      await Zotero.VibeDB.Papers.save(itemID, {
        resultDir: demoData.resultDir,
        markdownContent: demoData.markdownContent,
        articleSummary: null, // 旧字段，保持兼容
        outline: demoData.outline,
        blockMapping: demoData.blockMapping // ✅ 保存 blockMapping
      });

      var paper = await Zotero.VibeDB.Papers.get(itemID);
      var paperID = paper.paper_id;
      console.log('[Vibero] ✓ paper 保存成功, paper_id:', paperID);

      // 6. 保存 hierarchicalData（包含所有段落、points、sentences）
      if (demoData.hierarchicalData && Array.isArray(demoData.hierarchicalData)) {
        console.log('[Vibero] 开始保存 hierarchicalData...');

        // hierarchicalData 是按页面分组的数组，直接展开所有段落
        var allParagraphs = [];
        for (let pageData of demoData.hierarchicalData) {
          if (pageData.paragraphs && Array.isArray(pageData.paragraphs)) {
            // ✅ 现在每个 paragraph 都自带 pageIndex，无需额外处理
            allParagraphs.push(...pageData.paragraphs);
          }
        }

        // 转换为数据库格式（完全对称的逆过程）
        var paragraphsToSave = allParagraphs.map((p) => ({
          page_idx: p.pageIndex, // ✅ 直接使用 pageIndex（现在已包含在数据中）
          paragraph_idx: p.paragraphIndex,
          minerU_id: p.minerUid || null,
          paragraph_type: p.paragraphType || 'text',
          paragraph_text: p.paragraphText || p.text || null,
          paragraph_summary: p.paragraphSummary || p.summary || null,
          importance_level: p.importance_level || 1,
          bbox: p.bbox,
          rects: p.rects
        }));

        await Zotero.VibeDB.Paragraphs.saveBatch(itemID, paragraphsToSave, paperID);
        console.log('[Vibero] ✓ 段落保存成功');

        // 获取保存后的段落（带新的 paragraph_id）
        var savedParagraphs = await Zotero.VibeDB.Paragraphs.getByItemID(itemID, paperID);
        var paragraphIDMap = {};
        for (let para of savedParagraphs) {
          var key = `${para.page_idx}_${para.paragraph_idx}`;
          paragraphIDMap[key] = para.paragraph_id;
        }
        console.log('[Vibero] ✓ 构建了 paragraphIDMap, 共', Object.keys(paragraphIDMap).length, '个映射');

        // 保存 sentences 和 points（完全对称的逆过程）
        for (let oldPara of allParagraphs) {
          var key = `${oldPara.pageIndex}_${oldPara.paragraphIndex}`;
          var newParagraphID = paragraphIDMap[key];

          if (!newParagraphID) {
            console.warn('[Vibero] 找不到段落映射:', key);
            continue;
          }

          // 保存 sentences
          if (oldPara.sentences && oldPara.sentences.length > 0) {
            var sentencesToSave = oldPara.sentences.map((s) => ({
              sentence_idx: s.sentenceIndex,
              sentence_text: s.text,
              char_mapping: s.charMapping,
              start_char_offset: s.startCharIndex?.offset || null,
              end_char_offset: s.endCharIndex?.offset || null,
              rects: s.rects
            }));
            await Zotero.VibeDB.Sentences.saveBatch(newParagraphID, sentencesToSave);
          }

          // 保存 points
          if (oldPara.points && oldPara.points.length > 0) {
            var pointsToSave = oldPara.points.map((pt) => ({
              point_idx: pt.pointIndex,
              point_summary: pt.summary,
              point_translation: pt.translation,
              sentence_indices: pt.sentenceIndices,
              char_mapping: pt.charMapping,
              rects: pt.rects,
              importance_level: pt.importance_level || 1
            }));
            await Zotero.VibeDB.Points.saveBatch(newParagraphID, pointsToSave);
          }
        }
        console.log('[Vibero] ✓ sentences 和 points 保存成功');
      }

      // 7. 保存 sections
      if (demoData.sections) {
        console.log('[Vibero] 保存 sections...');
        await Zotero.VibeDB.Sections.saveBatch(paperID, demoData.sections);
        console.log('[Vibero] ✓ sections 保存成功');
      }

      // 8. 保存 article_summary
      if (demoData.articleSummary && demoData.articleSummary.length > 0) {
        console.log('[Vibero] 保存 article_summary...');
        await Zotero.VibeDB.ArticleSummary.saveBatch(paperID, demoData.articleSummary);
        console.log('[Vibero] ✓ article_summary 保存成功');
      }

      // 9. 保存 summaryCards
      if (demoData.summaryCards && demoData.summaryCards.length > 0) {
        console.log('[Vibero] 保存 summaryCards...');

        // 重新获取保存后的段落，用于查找 paragraph_id
        var savedParagraphs = await Zotero.VibeDB.Paragraphs.getByItemID(itemID, paperID);

        for (let card of demoData.summaryCards) {
          // 从 paragraphRef 中获取 pageIndex 和 paragraphIndex
          if (!card.paragraphRef) {
            console.warn('[Vibero] SummaryCard 缺少 paragraphRef:', card);
            continue;
          }

          var pageIndex = card.paragraphRef.pageIndex;
          var paragraphIndex = card.paragraphRef.paragraphIndex;

          // 查找对应的 paragraph_id
          var para = savedParagraphs.find((p) =>
          p.page_idx === pageIndex && p.paragraph_idx === paragraphIndex
          );

          if (!para) {
            console.warn('[Vibero] 找不到 SummaryCard 对应的段落:', pageIndex, paragraphIndex);
            continue;
          }

          // 保存 SummaryCard
          await Zotero.VibeDB.SummaryCards.save(itemID, {
            page_idx: card.position.pageIndex,
            paragraph_id: para.paragraph_id,
            summarycard_name: card.name || null,
            position_rects: card.position.rects
          });
        }
        console.log('[Vibero] ✓ summaryCards 保存成功');
      }

      // 10. 保存 flashCards
      if (demoData.flashCards && demoData.flashCards.length > 0) {
        console.log('[Vibero] 保存 flashCards...');

        // 重新获取保存后的段落，用于查找 paragraph_id
        var savedParagraphs = await Zotero.VibeDB.Paragraphs.getByItemID(itemID, paperID);

        for (let card of demoData.flashCards) {
          // FlashCard 可能没有关联段落
          var paragraphID = null;

          if (card.vibeCardRefs && card.vibeCardRefs.length > 0) {
            // 从 vibeCardRefs 中解析 pageIndex 和 paragraphIndex
            // vibeCardRefs 格式: ["vibecard_0_1"] -> pageIndex=0, paragraphIndex=1
            var refMatch = card.vibeCardRefs[0].match(/vibecard_(\d+)_(\d+)/);
            if (refMatch) {
              var pageIndex = parseInt(refMatch[1], 10);
              var paragraphIndex = parseInt(refMatch[2], 10);
          ...
          // 保存 FlashCard
          await Zotero.VibeDB.FlashCards.save(itemID, {
            page_idx: card.position.pageIndex,
        ...
        console.log('[Vibero] ✓ flashCards 保存成功');
      }

      console.log('[Vibero] ========== vibeDB 数据插入完成 ==========');
    }
    catch (error) {
      console.error('[Vibero] ❌ vibeDB 数据插入失败:', error);
      console.error('[Vibero] 错误堆栈:', error.stack);
      throw error;
    }
  };
  ...
  this.unserializePersist = function () {
    ...
    for (var id in serializedValues) {
      ...
      for (var attr in elValues) {
        ...
        // the splitter to behave unexpectedly. We set it to 'open' here (default open).
  ...
  /**
   * Toggle context pane (right sidebar)
   * Shortcut: Cmd/Ctrl+L
   */
  this.toggleContextPane = function () {
    ZoteroContextPane.collapsed = !ZoteroContextPane.collapsed;
  };

  /**
   * Toggle collections pane (left sidebar)
   * Shortcut: Cmd/Ctrl+B
   * 
   * 根据当前标签页类型切换不同的侧边栏：
   * - library 标签页：切换 collections pane
   * - reader 标签页：切换 reader 内的 sidebar
   */
  this.toggleCollectionsPane = function () {
    // 如果是 reader 标签页，切换 reader 的 sidebar
    if (Zotero_Tabs.selectedIndex > 0 && Zotero_Tabs.selectedType === 'reader') {
      let reader = Zotero.Reader.getByTabID(Zotero_Tabs.selectedID);
      if (reader) {
        // 调用 reader 的 toggleSidebar 方法
        reader.toggleSidebar();
        return;
      }
    }

    // 如果是 library 标签页，切换 collections pane
    var collectionsPane = document.getElementById('zotero-collections-pane');
    var collectionsSplitter = document.getElementById('zotero-collections-splitter');

    if (!collectionsPane || !collectionsSplitter) {
      return;
    }

    var isCollapsed = collectionsSplitter.getAttribute('state') === 'collapsed';

    if (isCollapsed) {
      // 展开
      collectionsPane.removeAttribute('collapsed');
      collectionsSplitter.setAttribute('state', 'open');
    } else {
      // 折叠
      collectionsPane.setAttribute('collapsed', 'true');
      collectionsSplitter.setAttribute('state', 'collapsed');
    }
  ...
  var _reloadObserver = {
    ...
    observe: function (aSubject, aTopic, aData) {
      ...
      if (aTopic == "zotero-before-reload") {
