...
var _reactDom = require("react-dom");var _reactDom2 = _interopRequireDefault(_reactDom);function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };} /*
	***** BEGIN LICENSE BLOCK *****
	...
	***** END LICENSE BLOCK *****
*/var tree;var engines;const columns = [{ dataKey: 'visible', type: 'checkbox', fixedWidth: true, width: 28 }, { dataKey: 'name', label: "zotero.preferences.locate.name" }, { dataKey: 'description', label: "zotero.preferences.locate.description" }];function init() {engines = Zotero.LocateManager.getEngines();const domEl = document.querySelector('#locateManager-tree');return new Promise((resolve) => {_reactDom2.default.createRoot(domEl).render(/*#__PURE__*/_react2.default.createElement(_virtualizedTable2.default, { getRowCount: () => engines.length, id: "locateManager-table", ref: (ref) => {tree = ref;resolve();}, renderItem: _virtualizedTable2.default.makeRowRenderer(getRowData),
            showHeader: true,
            multiSelect: true,
            columns: columns,
            onColumnSort: null,
