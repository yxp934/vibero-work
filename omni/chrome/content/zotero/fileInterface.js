...
var _citavi = require("zotero/import/citavi"); /*
    ***** BEGIN LICENSE BLOCK *****
    ...
    ***** END LICENSE BLOCK *****
*/var { FilePicker } = ChromeUtils.importESModule('chrome://zotero/content/modules/filePicker.mjs');ChromeUtils.defineESModuleGetters(globalThis, { HiddenBrowser: 'chrome://zotero/content/HiddenBrowser.mjs' }); /****Zotero_File_Exporter****
 **
 ...
 **/var Zotero_File_Exporter = function () {this.name = Zotero.getString("fileInterface.exportedItems");this.collection = false;this.items = false;}; /**
 * Performs the actual export operation
 *
 * @return {Promise}
 **/Zotero_File_Exporter.prototype.save = async function () {var translation = new Zotero.Translate.Export();var translators = await translation.getTranslators();translators.sort((a, b) => a.label.localeCompare(b.label)); // If exporting items, check whether they're only notes to determine which translators to show
  let exportingNotes = false;if (this.items) {exportingNotes = this.items.every((item) => item.isNote() || item.isAttachment());
