...
var CollectionTree = class CollectionTree extends LibraryTree {
  static async init(domEl, opts) {
    ...
    await new Promise((resolve) => {
      ReactDOM.createRoot(domEl).render(/*#__PURE__*/React.createElement(CollectionTree, _extends({ ref: (c) => {ref = c;resolve();} }, opts)));
    });
  ...
  handleKeyDown = (event) => {
    ...
    if (event.key == "End" && !this._isFilterEmpty()) {
      this.focusLastMatchingRow();
      return false;
    } else
    if (event.key == "Home" && !this._isFilterEmpty()) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        switch (type) {
          ...
          case 'group':
            ...
            row < this._rows.length && this.getLevel(row) > level);
      ...
      for (let id of ids) {
        switch (type) {
          ...
          case 'search':
            ...
            if (addedIndex !== false && addedIndex <= this.selection.focused) {
  ...
  */
  ...
  _matchesFilter(object, resetCache = false) {
    ...
    if (object.updateFeed && !object.libraryID) {
    ...
    if (object.libraryID && !["Search", "Feeds"].includes(object._ObjectType)) {
