...
var _itemTree = require("zotero/itemTree");var _itemTree2 = _interopRequireDefault(_itemTree);
var _itemTreeColumns = require("zotero/itemTreeColumns");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };} /*
    ***** BEGIN LICENSE BLOCK *****
    ...
    ***** END LICENSE BLOCK *****
*/var ZoteroAdvancedSearch = new function () {this.onLoad = onLoad;this.search = search;this.clear = clear;this.onItemActivate = onItemActivate;this.itemsView = false;var _searchBox;var _libraryID;async function onLoad() {_searchBox = document.getElementById('zotero-search-box'); // Set font size from pref
    var sbc = document.getElementById('zotero-search-box-container');Zotero.UIProperties.registerRoot(sbc);_searchBox.onLibraryChange = this.onLibraryChange;var io = window.arguments[0];io.dataIn.search.loadPrimaryData().
    then(function () {
      _searchBox.search = io.dataIn.search;
    });

    var elem = document.getElementById('zotero-items-tree');
    const columns = _itemTreeColumns.COLUMNS.map((column) => {
      column = Object.assign({}, column);
      column.hidden = !['title', 'firstCreator', 'year', 'hasAttachment'].includes(column.dataKey);
      return column;
    });
    this.itemsView = await _itemTree2.default.init(elem, {
      id: "advanced-search",
      dragAndDrop: true,
      persistColumns: true,
      columnPicker: true,
