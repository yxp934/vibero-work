...
var gCheckingPage = {
  ...
  async onPageShow() {
    ...
    try {
      ...
      try {
        let updateUrl = Services.urlFormatter.formatURLPref("app.update.url");
        console.log("Current app.update.url: " + updateUrl);
      } catch (e) {
        console.log("Could not format app.update.url: " + e);
      }
      try {
        let customUrl = Zotero.Prefs.get("extensions.zotero.vibeZotero.updateUrl");
        console.log("Custom vibeZotero.updateUrl: " + customUrl);
      } catch (e) {
        console.log("Could not get vibeZotero.updateUrl: " + e);
      }
      console.log("=========================================");
...
var gDownloadingPage = {
  ...
  async onPageShow() {
    ...
    if (activeUpdate) {
      ...
      if (activeUpdate.state == STATE_APPLIED ||
