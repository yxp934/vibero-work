...
var _icons = require("components/icons");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}var { FilePicker } = ChromeUtils.importESModule('chrome://zotero/content/modules/filePicker.mjs');

Services.scriptloader.loadSubScript('chrome://zotero/content/elements/styleConfigurator.js', undefined);
...
const Zotero_RTFScan = { // eslint-disable-line no-unused-vars, camelcase
  ...
  async init() {
    ...
    _reactDom2.default.createRoot(document.getElementById('tree')).render(/*#__PURE__*/
    ...
    for (const column of columns) {
      if (column.primary) {
        ...
        if (row.children || this.rows[index + 1] && this.rows[index + 1].parent == row) {
          twisty = (0, _icons.getCSSIcon)("twisty");
      ...
      if (column.dataKey == 'action') {
        ...
        if (row.parent) {
          ...
          let icon = (0, _icons.getCSSIcon)(row.action ? 'document-accept' : 'link');
