...
var ItemTree = class ItemTree extends LibraryTree {
  ...
  refresh = Zotero.serial(async function (skipExpandMatchParents) {
    ...
    try {
      ...
      console.log('[ItemTree.refresh] 准备 resolve _refreshPromise');
      setTimeout(function () {
        console.log('[ItemTree.refresh] <<< refresh resolve 执行');
        resolve();
      });
    }
    catch (e) {
  ...
  render() {
    ...
    let virtualizedTable = /*#__PURE__*/React.createElement("div", { style: { display: showMessage ? 'none' : 'flex' },
      className: "virtualized-table focus-states-target", key: "virtualized-table-stub" });
    if (this.collectionTreeRow) {
  ...
  async changeCollectionTreeRow(collectionTreeRow) {
    ...
    console.log('[ItemTree] >>> changeCollectionTreeRow 开始, collectionId:', collectionTreeRow.id);
    // ✅ 防止竞态条件：使用本地引用保存 deferred
    const localDeferred = Zotero.Promise.defer();
    this._itemTreeLoadingDeferred = localDeferred;
    ...
    this.forceUpdate(() => {
      ...
      this._updateIntroText();
      // ✅ 使用本地引用，确保 resolve 正确的 promise
      localDeferred.resolve();
    });
    // ✅ 等待本地保存的 promise，而不是可能被覆盖的 this._itemTreeLoadingDeferred.promise
  ...
  _renderItem(index, selection, oldDiv = null, columns) {
    ...
    if (item.isAnnotation()) {
      ...
      if (["highlight", "underline", "card"].includes(item.annotationType)) {
