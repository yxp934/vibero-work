...
var _progressQueueTable = require("components/progressQueueTable");var _progressQueueTable2 = _interopRequireDefault(_progressQueueTable);function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };} /*
	...
	***** END LICENSE BLOCK *****
*/ /* eslint camelcase: ["error", {allow: ["Zotero_File_Interface", "Zotero_Import_Wizard"]} ] */ /* global Zotero_File_Interface: false, mendeleyAPIUtils: false */var { FilePicker } = ChromeUtils.importESModule('chrome://zotero/content/modules/filePicker.mjs');Services.scriptloader.loadSubScript("chrome://zotero/content/import/mendeley/mendeleyAPIUtils.js");const { directAuth } = mendeleyAPIUtils;const Zotero_Import_Wizard = { // eslint-disable-line no-unused-vars
  file: null, folder: null, isZotfileInstalled: false, libraryID: null, mendeleyAuth: null, mendeleyCode: null, mendeleyHasPreviouslyImported: false, mendeleyImporterVersion: 0, translation: null, wizard: null, async getShouldCreateCollection() {const sql = "SELECT ROWID FROM collections WHERE libraryID=?1 " + "UNION " + "SELECT ROWID FROM items WHERE libraryID=?1 " // Not in trash
    + "AND itemID NOT IN (SELECT itemID FROM deletedItems) " // And not a child item (which doesn't necessarily show up in the trash)
    + "AND itemID NOT IN (SELECT itemID FROM itemNotes WHERE parentItemID IS NOT NULL) " + "AND itemID NOT IN (SELECT itemID FROM itemAttachments WHERE parentItemID IS NOT NULL) " +
    "LIMIT 1";
  ...
  skipToDonePage(label, description, showReportErrorButton = false, isMendeleyError = false) {
    ...
    if (this.folder && !showReportErrorButton) {
      ...
      _reactDom2.default.createRoot(doneQueue).render(/*#__PURE__*/
        _react2.default.createElement(_progressQueueTable2.default, { progressQueue: Zotero.ProgressQueues.get('recognize') })
      );
    } else
    {
  ...
  async onMendeleyOnlineAdvance(ev) {
    ...
    } else
    {
      ...
      try {
        ...
        if (e instanceof Zotero.HTTP.UnexpectedStatusException && (e.status === 400 || e.status === 401 || e.status === 403)) {
  ...
  async onProgressPageShow() {
    ...
    if (this.folder) {
      ...
      _reactDom2.default.createRoot(progressQueue).render(/*#__PURE__*/
        _react2.default.createElement(_progressQueueTable2.default, { progressQueue: Zotero.ProgressQueues.get('recognize') })
      );
    } else
    {
