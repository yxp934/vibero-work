...
Zotero_Preferences.Export = {
  ...
  populateNoteQuickCopyList: async function () {
    ...
    translators.forEach(function (translator) {
      ...
      if (translator.translatorID == Zotero.Translators.TRANSLATOR_ID_MARKDOWN_AND_RICH_TEXT) {
        value = {
          mode: 'export',
      ...
      if (translator.translatorID == Zotero.Translators.TRANSLATOR_ID_NOTE_HTML) {
        value = {
          mode: 'export',
          id: translator.translatorID,
          options: {
  ...
  refreshQuickCopySiteList: async function () {
    ...
    if (!this._tree) {
      ...
      await new Promise((resolve) => {
        ReactDOM.createRoot(document.getElementById("quickCopy-siteSettings")).render(/*#__PURE__*/
