...
function executeSearch(context, term, options) {
  ...
  while (matches = re.exec(text)) {
    let [match] = matches;
    let {
      charDataID: startCharDataID,
      start: startOffset
    } = binarySearch(internalCharDataRanges, matches.index);
    let {
      charDataID: endCharDataID,
      start: endOffset
    } = binarySearch(internalCharDataRanges, matches.index + match.length);
    ranges.push({
      startCharDataID,
